...
#if USE_BLAS
CPUData SkipConvLayer::m_im2col;
CPUData SkipConvLayer::m_output_buffer;
#endif

SkipConvLayer::SkipConvLayer(std::istream &stream, std::vector<std::string> &input_links)
	: Layer(stream, input_links)
{
	...
	m_skip_x = ReadData<int>(stream);
	m_skip_y = ReadData<int>(stream);
...
void SkipConvLayer::Dump(std::ostream &stream)
{
	Layer::Dump(stream);
	...
	WriteData(stream, m_skip_x);
	WriteData(stream, m_skip_y);
...
SkipConvLayer::~SkipConvLayer()
{
	SaveRelease(m_weight);
	SaveRelease(m_biases);
}

int SkipConvLayer::GetInputPadX()
{
	return m_padding_x;
}

int SkipConvLayer::GetInputPadY()
{
	return m_padding_y;
}

void SkipConvLayer::InitActs()
{
	...
	int output_width = (input->GetWidth() + 2 * m_padding_x - (m_weight->GetWidth() -  1) * m_skip_x - 1) / m_stride_x + 1;
	int output_height = (input->GetHeight() + 2 * m_padding_y - (m_weight->GetHeight() - 1) * m_skip_y - 1) / m_stride_y + 1;
...
void SkipConvLayer::Forward()
{
	...
	for (int n = 0; n < input->GetNum(); ++n)
	{
		...
		// im2col
		TransIm2ColSkip(data, input_channels, input_stride, filter_width, filter_height, m_stride_x, m_stride_y, m_skip_x, m_skip_y,
