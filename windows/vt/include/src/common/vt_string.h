...
#if defined(VT_GCC) && defined(_WIN32)
extern "C" int vswprintf(wchar_t *wcs, size_t maxlen,
...
struct char_traits<char>
{
    ...
    static void copy(char* _U, size_t /*_NU*/, 
		                     const char* _V, size_t _N)
        {memcpy(_U, _V, _N); }
...
public:
    ...
    HRESULT format_with_resize(_Printf_format_string_ const T* pszFmt, ...)
    {
		...
		va_list Args;
		va_start(Args, pszFmt);
		size_t bufferlen = (size_t)_Tr::format_buffer_size(pszFmt, Args) + 1;
        if( _Res < bufferlen )
		{
			hr = _Grow(bufferlen);
		}
...
protected:
    ...
    public:
        ...
        long _Release()
        {
            if(VT_INTERLOCKED_DECREMENT(&_RefCount) == 0)
            {
                delete this;
                return 0;
            }
