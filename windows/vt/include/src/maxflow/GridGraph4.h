...
*
*
*   Nodes are referenced using struct GridGraph4<NLinkFlowType,TotalFlowType>::NodeId. Nodes are stored in a row-by-row order.
*   Below is an example of fast pass over nodes:
*
*   GridGraph4<int,int>::NodeId p = graph->GetNodeId(0, 0);
...
const int gridGraph4XShifts[4] = { 1, 0, -1,  0};
const int gridGraph4YShifts[4] = { 0, 1,  0, -1};


template <class NLinkFlowType,class TotalFlowType> 
  struct GridGraph4<NLinkFlowType,TotalFlowType>::Square
{
...
template <class NLinkFlowType,class TotalFlowType> 
  struct GridGraph4<NLinkFlowType,TotalFlowType>::VirtualGraph2
{
    void Clear();
...
template <class NLinkFlowType,class TotalFlowType> 
  inline void GridGraph4<NLinkFlowType,TotalFlowType>::AddTEdge(NodeId p, TLinkFlowType w)
{
    ASSERT(IsAllocated());
...
template <class NLinkFlowType,class TotalFlowType> 
  inline void GridGraph4<NLinkFlowType,TotalFlowType>::AddNEdge(NodeId p, int edge, NLinkFlowType w)
{
    ASSERT(IsAllocated());
    ASSERT(p>=0 && p<m_nodeNum);
    ASSERT(GetX(p)+gridGraph4XShifts[edge]>=0 && GetX(p)+gridGraph4XShifts[edge]<m_sizeX);
    ASSERT(GetY(p)+gridGraph4YShifts[edge]>=0 && GetY(p)+gridGraph4YShifts[edge]<m_sizeY);
    m_nodes[p].m_nCap[edge] = (NLinkFlowType)(m_nodes[p].m_nCap[edge] + w);
}
template <class NLinkFlowType,class TotalFlowType> 
  inline typename GridGraph4<NLinkFlowType,TotalFlowType>::TLinkFlowType GridGraph4<NLinkFlowType,TotalFlowType>::GetTEdge(NodeId p)
{
    ASSERT(IsAllocated());
    ASSERT(p>=0 && p<m_nodeNum);
...
template <class NLinkFlowType,class TotalFlowType> 
  inline typename NLinkFlowType GridGraph4<NLinkFlowType,TotalFlowType>::GetNEdge(NodeId p, int edge)
{
    ASSERT(IsAllocated());
    ASSERT(p>=0 && p<m_nodeNum);
    ASSERT(GetX(p)+gridGraph4XShifts[edge]>=0 && GetX(p)+gridGraph4XShifts[edge]<m_sizeX);
...
template <class NLinkFlowType,class TotalFlowType> 
  inline void GridGraph4<NLinkFlowType,TotalFlowType>::AddNEdges(NodeId p, int edge, NLinkFlowType w, NLinkFlowType w_rev)
{
    ASSERT(IsAllocated());
    ASSERT(p>=0 && p<m_nodeNum);
    ASSERT(GetX(p)+gridGraph4XShifts[edge]>=0 && GetX(p)+gridGraph4XShifts[edge]<m_sizeX);
...
template <class NLinkFlowType,class TotalFlowType> 
  inline int GridGraph4<NLinkFlowType,TotalFlowType>::GetSegmentation(NodeId p)
{
    ASSERT(IsAllocated());
    ASSERT(p>=0 && p<m_nodeNum);
...
template <class NLinkFlowType,class TotalFlowType> 
  inline void GridGraph4<NLinkFlowType,TotalFlowType>::SetSquare(int x, int y, Square& s)
{
    s.m_x = x;
    s.m_y = y;
    s.m_left_top = GetNodeId(x, y);
    s.m_left_bottom = GetNeib(s.m_left_top,1);
    s.m_right_top = GetNeib(s.m_left_top,0);
    s.m_right_bottom = GetNeib(s.m_left_bottom,0);

}

template <class NLinkFlowType,class TotalFlowType> 
  inline void GridGraph4<NLinkFlowType,TotalFlowType>::MoveSquareRight(Square& s)
{
    ASSERT(GetNodeId(s.m_x, s.m_y) == s.m_left_top);
    ASSERT(s.m_left_bottom==GetNeib(s.m_left_top,1) && s.m_right_top==GetNeib(s.m_left_top,0) && s.m_right_bottom==GetNeib(s.m_left_bottom,0));
    s.m_x ++;
...
template <class NLinkFlowType,class TotalFlowType> 
  inline void GridGraph4<NLinkFlowType,TotalFlowType>::MoveSquareDown(Square& s)
{
    ASSERT(GetNodeId(s.m_x, s.m_y) == s.m_left_top);
    ASSERT(s.m_left_bottom==GetNeib(s.m_left_top,1) && s.m_right_top==GetNeib(s.m_left_top,0) && s.m_right_bottom==GetNeib(s.m_left_bottom,0));
    s.m_y ++;
    s.m_left_top = s.m_left_bottom;
    s.m_left_bottom = GetNeib(s.m_left_bottom, 1);
    s.m_right_top = s.m_right_bottom;
    s.m_right_bottom = s.m_left_bottom + 1;
}

template <class NLinkFlowType,class TotalFlowType> 
  inline void GridGraph4<NLinkFlowType,TotalFlowType>::MoveSquareLeft(Square& s)
{
    ASSERT(GetNodeId(s.m_x, s.m_y) == s.m_left_top);
    ASSERT(s.m_left_bottom==GetNeib(s.m_left_top,1) && s.m_right_top==GetNeib(s.m_left_top,0) && s.m_right_bottom==GetNeib(s.m_left_bottom,0));
    s.m_x --;
...
template <class NLinkFlowType,class TotalFlowType> 
  inline void GridGraph4<NLinkFlowType,TotalFlowType>::MoveSquareUp(Square& s)
{
    ASSERT(GetNodeId(s.m_x, s.m_y) == s.m_left_top);
    ASSERT(s.m_left_bottom==GetNeib(s.m_left_top,1) && s.m_right_top==GetNeib(s.m_left_top,0) && s.m_right_bottom==GetNeib(s.m_left_bottom,0));
    s.m_y --;
    s.m_left_bottom = s.m_left_top;
    s.m_left_top = GetNeib(s.m_left_top, 3);
