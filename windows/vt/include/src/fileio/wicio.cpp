...
WIC_PIXFRMT_MAP g_WICPixelFormatMap[] =
{
    ...
    {&GUID_WICPixelFormat1bppIndexed, OBJ_UNDEFINED, OBJ_RGBIMG, false},
    {&GUID_WICPixelFormat2bppIndexed, OBJ_UNDEFINED, OBJ_RGBIMG, false},
    {&GUID_WICPixelFormat4bppIndexed, OBJ_UNDEFINED, OBJ_RGBIMG, false},
    {&GUID_WICPixelFormat8bppIndexed, OBJ_UNDEFINED, OBJ_RGBIMG, false},
...
static HRESULT WICInitializeFactory(IWICImagingFactory** ppFactory)
{
	...
	if (*ppFactory == NULL)
	{
		hr = CSystem::CoCreateInstance(CLSID_WICImagingFactory, NULL,
			CLSCTX_INPROC_SERVER, IID_IWICImagingFactory, (void**)ppFactory);
	}
	return hr;
}
                           ...
                           const pMetadataReader, CParams& params)
{
	...
	for (UINT nIndex = 0; nIndex < nCount; nIndex++)
    {
		...
		if (SUCCEEDED(hr))
		{
			...
			CComPtr<IWICMetadataReader> pSubMetadataReader;
...
VtCopyWICFrameMetadata(IWICBitmapFrameDecode* const pBitmapFrame, CParams& params)
{
    ...
    CComPtr<IWICMetadataBlockReader> pMetadataBlockReader;
    ...
    for(UINT idx = 0; idx < count; idx++)
    {
        ...
        ATL::CComPtr<IWICMetadataReader> pMetadataReader;
                      ...
                      CParams& params)
{
    ...
    {
        vt::vector<CComPtr<IWICColorContext>> vecCC;
            ...
            uCCCnt, (IWICColorContext**)vecCC.begin(), &uCCCnt) == S_OK )
        {
            for( UINT i = 0; i < uCCCnt; i++ )
            {
                CComPtr<IWICColorContext>& pCC = vecCC[i];
                ...
                pCC->GetProfileBytes(0, NULL, &uProfileByteCount);
                                   ...
                                   PROPVARIANT &varValue)
{
    ...
    if(varValue.vt == VT_LPSTR || varValue.vt == VT_LPWSTR)
    {
        ...
        memcpy(varValue.pszVal, pVtVal->GetDataPtr(), cSize);
    }
                         ...
                         IWICBitmapFrameEncode* pFrame)
{
    ...
    {
        CComPtr<IWICColorContext> pColorContext;
        if (SUCCEEDED( pFactory->CreateColorContext(&pColorContext) ))
        {
            ...
            {
                pFrame->SetColorContexts(1, &pColorContext.p);
            }
        }
    }
...
{
    HRESULT hr;
    CComPtr<IWICFormatConverter> pFormatConvert;
...
HRESULT CWicReader::OpenFile(IStorageFile^ storageFile)
{
	...
	try
	{
		...
		CComPtr<IUnknown> comBuffer(reinterpret_cast<IUnknown*>(buffer));
		CComPtr<IBufferByteAccess> bufferByteAccess;
...
CWicReader::LoadFrame()
{
    ...
    {
        m_info = CImgInfo((int)uWidth, (int)uHeight, OBJ_RGBFLOATIMG);
    }
    else
    {
        m_info = CImgInfo((int)uWidth, (int)uHeight, pmap->iPreferType);
    }
                       ...
                       const CRect* pRect)
{
    ...
    CComPtr<IWICBitmapSource> pBitmapSource = m_pBitmapFrame;
    ...
    if ( pMap->iMatchType == OBJ_UNDEFINED )
    {
        ...
        CImgInfo imgDst(uWidth, uHeight, pMap->iPreferType);
...
#if defined(VT_WINRT)
HRESULT
CWicWriter::OpenFile( IStorageFile^ storageFile )
{
	VT_HR_BEGIN();
	...
	VT_HR_EXIT(OpenFileInternal(NULL, tempStorageFile->Path->Data()));
...
CWicWriter::SetImage( IStream* pStream, CRect& rect, const CParams* pParams )
{
    ...
    CComPtr<IWICBitmapFrameDecode> pFrameDecoder;
    ...
    pFrameDecoder->GetPixelFormat(&guidFormatSrc);
    double dpiX, dpiY;
...
CWicMetadataWriter::SaveMetadata()
{
    ...
    CComPtr<IWICPersistStream> pWICStream;
    VT_HR_EXIT( pWriter->QueryInterface(
        IID_IWICPersistStream, (void **) &pWICStream) );
        ...
        while( pEnum->Next(1, &pStr, NULL) == S_OK )
        {
            if( pStr )
            {
                ...
                if( value.vt == VT_UNKNOWN )
                {
                    ...
                    CComPtr<IWICMetadataQueryReader> pMetaNested;
