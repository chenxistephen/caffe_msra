...
// 
// Synposis: an IIndexedImageReaderWriter that wraps a blob (.vti file)
//
//------------------------------------------------------------------------------
...
CVtiReaderWriter::ComputeSourceSize()
{
    ...
    for (UINT uIndex = 0; uIndex < m_vecInfos.size(); uIndex++)
    {
        ...
        for (UINT uLevel = 0; uLevel < m_vecBlocks[uIndex].size(); uLevel++)
        {
                             ...
                             UINT uTractSize)
{
    ...
    for (UINT uIndex = 0; uIndex < vecInfos.size(); uIndex++)
    {
        VTI_INFO info;
