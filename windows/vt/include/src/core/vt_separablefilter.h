...
// 
// Synposis: IImageTransform implementation of VtSeparableFilter121
// 
/// \ingroup filtertransforms
...
public:
	...
	///		- always returns S_OK
	/// </returns>
    HRESULT Initialize(void) { m_NoRW = false; m_bDeci = false; return S_OK; };
	/// <summary> Initialize transform for 2 to 1 decimation </summary> 
	/// <returns> 
	///		- always returns S_OK
	/// </returns>
    HRESULT InitializeDecimate2to1(void) { m_NoRW = false; m_bDeci = true; return S_OK; };

    // no-copy initializaton routines - use with CTransformGraphNoSrcNode
    HRESULT Initialize(CImg& dst, CImg& src)
    { m_NoRW = true; dst.Share(m_dstImg); src.Share(m_srcImg); m_bDeci = false; return S_OK; }
    HRESULT InitializeDecimate2to1(CImg& dst, CImg& src)
    { m_NoRW = true; dst.Share(m_dstImg); src.Share(m_srcImg); m_bDeci = true; return S_OK; }
...
// 
// Synposis: Specialization of CSeparableFilterTransform for a 14641 filter
// kernel.  Currently only supports upsampling by a two-to-one ratio.
// 
/// \ingroup filtertransforms
/// <summary>Implementation of \link IImageTransform image transform \endlink
/// that applies a 14641 filter with two-to-one upsampling.
/// </summary> 
//------------------------------------------------------------------------------
class CSeparableFilter14641Transform: 
...
public:
	...
	vt::CRect GetRequiredSrcRect(const vt::CRect& rctDst)
	{ return vt::GetRequiredSrcRect(rctDst, m_ksUp, m_ksUp); }

	vt::CRect GetAffectedDstRect(const vt::CRect& rctSrc)
	{ return vt::GetAffectedDstRect(rctSrc, m_ksUp, m_ksUp); }

	vt::CRect GetResultingDstRect(const vt::CRect& rctSrc)
	{
		const float factorX = m_ksUp.GetCycle() /
			((float)m_ksUp.GetCoordShiftPerCycle());  
		const float factorY = m_ksUp.GetCycle() /
			((float)m_ksUp.GetCoordShiftPerCycle());
		return vt::CRect(F2I(rctSrc.left * factorX), F2I(rctSrc.top * factorY), 
			F2I(rctSrc.right * factorX), F2I(rctSrc.bottom * factorY)); 
	}
...
public:
	...
	/// <summary> Initialize transform for two-to-one upsampling. </summary> 
	/// <returns> 
	///		- S_OK on success 
	///		- E_OUTOFMEMORY if memory allocation failed 
	/// </returns>
    HRESULT InitializeUpsample2to1(int dstType);
