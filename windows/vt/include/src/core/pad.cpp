							   ...
							   const vt::CRect& rctOOB, const vt::CRect& rctSrc)
{
	...
	{
		rctInB.left  = VtMax(0L,rctOOB.left); 
	...
	{
		rctInB.top    = VtMax(0L,rctOOB.top); 
							...
							const vt::CPoint* pptDst)
{
	...
	long iW = infoReader.width;
	long iH = infoReader.height;
...
HRESULT ConvertConstval(EXTEND_CONSTVAL& excDst, const CImgInfo& imgDst,
                        const EXTEND_CONSTVAL& excSrc)
{
    VT_HR_BEGIN()

	VT_HR_EXIT( excDst.Alloc(imgDst.type) );

	if (VT_IMG_BANDS(excSrc.iType) == 1 && imgDst.Bands() > 1)
    {
    	EXTEND_CONSTVAL excTmp;
    	VT_HR_EXIT( excTmp.Alloc(VT_IMG_MAKE_TYPE(EL_FORMAT(excSrc.iType),imgDst.Bands())) );
		VtFillSpan(excTmp.Val(), excSrc.Val(), VT_IMG_ELSIZE(excTmp.iType), imgDst.Bands());

        VT_HR_EXIT( VtConvertSpan(excDst.Val(), imgDst.type,
							   ...
							   const IMAGE_EXTEND& ex)
{
	...
	CRect rctSrc   = infoSrc.Rect();
	bool bEmptySrc = rctSrc.IsRectEmpty();
	if( bEmptySrc && !rctSrc.RectInRect(&rRequest) &&
		(!ExtendModeAllowsEmptySrc(ex.exHoriz) || 
		 !ExtendModeAllowsEmptySrc(ex.exVert)) )
	{
		return E_INVALIDARG;
	}

	// source needs to be present if source rect is
	if ( !bEmptySrc && !IsValid(pSrc) )
		return E_INVALIDSRC;
	
	VT_HR_BEGIN()

	// create the dest image if it hasn't already been
	VT_HR_EXIT( CreateImageForTransform(imgDst, rRequest.Width(), rRequest.Height(),
										infoSrc.type) );  
  
	// convert extend type const values if necessary
    // Check if the extend modes are typemax/min then set accordingly
	EXTEND_CONSTVAL excDstH, excDstV;
	const EXTEND_CONSTVAL *pexcDstH, *pexcDstV;
	if( ex.exHoriz==Constant && 
		(EL_FORMAT(ex.excHoriz.iType) != EL_FORMAT(imgDst.GetType()) ||
		VT_IMG_BANDS(ex.excHoriz.iType) != imgDst.Bands()) )
	{
        VT_HR_EXIT( ConvertConstval(excDstH, imgDst.GetImgInfo(), ex.excHoriz) );
        pexcDstH = &excDstH;
    }
    else
    {
        pexcDstH = &ex.excHoriz;
    }

	// TODO: same code as above - probably should be a separate re-usable function
    if( ex.exVert==Constant && 
        (EL_FORMAT(ex.excVert.iType) != EL_FORMAT(imgDst.GetType()) ||
         VT_IMG_BANDS(ex.excVert.iType) != imgDst.Bands()) )
    {
        VT_HR_EXIT( ConvertConstval(excDstV, imgDst.GetImgInfo(), ex.excVert) );
        pexcDstV = &excDstV;
    }
    else
    {
    ...
    for( UInt32 i = 0; i < uOpsCnt; i++ )
    {
        ...
        case ePadOpConstantHoriz:
            ...
            {
                VT_HR_EXIT( imgDstOp.Fill((Byte*)pexcDstH->Val()) );
            }
            break;
        case ePadOpConstantVert:
            ...
            {
                VT_HR_EXIT( imgDstOp.Fill((Byte*)pexcDstV->Val()) );
            }
            break;
        case ePadOpPad:
        case ePadOpPadNonIntersect:
            {
                ...
                VtExtendConstZeroPadImage(imgDstOp, rctWrite, 
                    ex.exHoriz, ex.exVert, 
                    pexcDstH->Val(), pexcDstV->Val());
			}
			break;
		}
	}
