...
struct EXTEND_CONSTVAL
{
	...
	EXTEND_CONSTVAL() : uSize(0), pv(NULL), iType(OBJ_UNDEFINED)
	{}
...
struct IMAGE_EXTEND
{
    ...
    {
        return !(exHoriz==Wrap || exHoriz==Extend || exHoriz==ExtendZeroAlpha ||
                 exVert==Wrap  || exVert==Extend  || exVert==ExtendZeroAlpha);
    }
