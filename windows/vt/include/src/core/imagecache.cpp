...
// Note: disable /analyze 6011 warnings in this file because there are alot
//       of false positives
...
#if defined(VT_WINRT)
    m_pFileStore = VT_NOTHROWNEW CWinRTBlobStore;
#else
...
CImageCache::~CImageCache()
{
    for (UINT i = 0; i < m_vecSources.size(); i++)
    {
    ...
    while (m_hBackingThread != NULL)
    {
            ...
            m_cBackingReads < MAXIMUM_WAIT_OBJECTS; pBlock = m_pTailReads)
        {
            ...
            if (pBlock->pSource->hrBackingResult == S_OK)
                pBlock->pSource->hrBackingResult = BlobBlock(pBlock, false);
            else
            {
                ...
                m_uMemoryUsed - m_uMemoryPending > m_uHighWater)
            {
                ...
                if (pBlock->dwThreadId == 0 && pBlock->dwThreadCount == 0)
                {
                    ...
                    if (pBlock->pCallback == NULL)
                    {
                        ...
                        {
                            if (pBlock->pSource->hrBackingResult == S_OK)
                                pBlock->pSource->hrBackingResult = BackupBlock(pBlock);
                        }
                    }
                }
            }
                       ...
                       const IMG_CACHE_SOURCE_PROPERTIES* pProps)
{
    ...
    for (UINT uIndex = 0; uIndex < source->vecImages.size(); uIndex++)
    {
    ...
    // Find empty source.
    for (uId = 0; uId < m_vecSources.size(); uId++)
    {
        if (m_vecSources[uId] == NULL)
            break;
    }
    // If no empty source found add one.
    if (uId < m_vecSources.size())
...
HRESULT
CImageCache::FlushSource(UINT uSourceId)
{
    HRESULT hr = S_OK;
...
CImageCache::DeleteSource(UINT uSourceId)
{
    ...
    for (uSourceId = 0; uSourceId < m_vecSources.size(); uSourceId++)
...
HRESULT
CImageCache::FlushSource(SOURCE* source)
{
    HRESULT hr = S_OK;
    ...
    do
    {
        ...
        for( UINT uIndex = 0; uIndex < source->vecImages.size(); uIndex++)
        {
            ...
            for( UINT uLevel = 0; uLevel < image.vecLevels.size(); uLevel++)
            {
        ...
        bBlocksInBlob = true;
        for (UINT uIndex = 0; uIndex < source->vecImages.size() && bBlocksInBlob; uIndex++)
        {
            IMAGE& image = source->vecImages[uIndex];

            for (UINT uLevel = 0; uLevel < image.vecLevels.size() && bBlocksInBlob; uLevel++)
            {
                LEVEL& level = image.vecLevels[uLevel];

                for (UINT uBlock = 0; uBlock < level.vecBlocks.size() && bBlocksInBlob; uBlock++)
                {
...
CImageCache::BlobBlock(LINK* pBlock, bool bWrite)
{
    ...
    UINT uTractNum =
        (pBlock->pBlock->uTractNum & ~(BLOCK_IN_STORE | BLOCK_IN_CACHE)) *
    ...
    Byte *pbData = (Byte *) (((UINT_PTR) pBlock->pbAlloc +
        uPageMask) & ~uPageMask);
...
#define QUEUE_MASK (QUEUE_SIZE - 1)
        ...
        !bi.Done(); bi.Advance())
    {
        if ((hr = DoBlock(source, uIndex, rect, img, uLevel,
                          bWrite, bWriteThrough, uClearVal, bi.GetCompRect())) == S_FALSE)
        {
            // Grow queue by QUEUE_SIZE if necessary.
            if (cPending > 0 && (cPending & QUEUE_MASK) == 0)
            {
                     ...
                     UINT uClearVal, IN const CRect& rctBlk)
{
        ...
        (block.uTractNum & BLOCK_IN_CACHE) != 0x0)
    {
        ...
        else
        {
            ...
            // Round up to multiple of page size.
            UINT_PTR uPageMask = m_uPageSize - 1;
            Byte *pbData = (Byte *) (((UINT_PTR) block.pLink->pbAlloc +
                uPageMask) & ~uPageMask);
            VT_HR_EXIT( imgBlk.Create(pbData, rctBlk.Width(), rctBlk.Height(),
                         ...
                         UINT uClearVal)
{
    ...
    for (uLevel = uLevel + 1; uLevel < image.vecLevels.size(); uLevel++)
    {
        LEVEL& level = image.vecLevels[uLevel];
...
CImageCache::CreateBlob(SOURCE* source)
{
    ...
    for (UINT i = 0; i < source->vecImages.size(); i++)
