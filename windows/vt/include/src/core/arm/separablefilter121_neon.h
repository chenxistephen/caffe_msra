    ...
    const Byte* pT, const Byte* pC, const Byte* pB )
{
    ...
    while ( w >= 8 )
    {
        ...
        x2 = vld1q_u8(pC); pC += 16;
        x3 = vld1q_u8(pB); pB += 16;
        x1 = vhaddq_u8( x1, x3 ); // T+B
        x1 = vhaddq_u8( x1, x2 ); // T+C+C+B
        ...
        x3 = vhaddq_u8( x2, x3 ); // L+R
        x3 = vhaddq_u8( x3, x1 ); // L+C+C+R
        __n128x2 x4 = vuzpq_u8( x3, x3 );
        vst1_u8(pDst, x4.val[0].s.low64); pDst += 8;
    ...
    const Byte* pT, const Byte* pC, int srcStride)
{
    ...
    while ( w >= 8 )
    {
        uint8x16_t C0,C1,C2,C3;
        {
            ...
            x4 = vld1q_u8(pCT );
            x0 = vhaddq_u8( x0, x1 ); // T+B
            C0 = vhaddq_u8( C0, x0 ); // T+C+C+B
            x0 = vhaddq_u8( x1, x2 ); // T+B
            C1 = vhaddq_u8( C1, x0 ); // T+C+C+B
            x0 = vhaddq_u8( x2, x3 ); // T+B
            C2 = vhaddq_u8( C2, x0 ); // T+C+C+B
            x0 = vhaddq_u8( x3, x4 ); // T+B
            C3 = vhaddq_u8( C3, x0 ); // T+C+C+B
        }

        {
            ...
            x0 = vhaddq_u8( x0, x1 ); // L+R
            x0 = vhaddq_u8( C0, x0 ); // L+C+C+R
            {
                __n128x2 x3 = vuzpq_u8( x0, x0 );
                vst1_u8(pDst, x3.val[0].s.low64);
            }
        }
        Byte* pDstT = pDst+dstStride;
        {
            ...
            x0 = vhaddq_u8( x0, x1 ); // L+R
            x0 = vhaddq_u8( C1, x0 ); // L+C+C+R
            {
                __n128x2 x3 = vuzpq_u8( x0, x0 );
                vst1_u8(pDstT, x3.val[0].s.low64);
            }
        }
        pDstT += dstStride;
        {
