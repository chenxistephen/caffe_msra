...
public: 
	...
	//
	//--------------------------------------------------------------------------
	HRESULT FlushSource(UINT uSourceId);
...
#define TRACT_MASK (UINT_MAX & ~(BLOCK_IN_STORE | BLOCK_IN_CACHE))
    ...
    struct IMAGE
    {
        ...
        ~IMAGE()
        {
            ...
            // Delete clear values.
            for (UINT uVal = 0; uVal < vecClearVals.size(); uVal++)
    ...
    struct LINK
    {
        ...
        LINK() : pSource(NULL), pbAlloc(NULL), cbAlloc(0), cbData(0),
...
private:
    ...
    HRESULT AddSource(SOURCE* source,  OUT UINT& uSourceId,
                      ...
                      const IMG_CACHE_SOURCE_PROPERTIES* pProps = NULL);
    HRESULT FlushSource(SOURCE* source);
...
public:
	...
	HRESULT PushBack(const wstring wstrBackingBlob, bool bReadOnly = false)
	{
        if (m_uSourceId != UINT_MAX)
            return E_ACCESSDENIED;
	...
	{ 
		if ( m_pIC )
		{
			...
			for ( UINT i = 0; i < m_vSourceIds.size(); i++ )
			{
...
public:
	HRESULT PushBack(int iW, int iH, 
					 const IMG_CACHE_SOURCE_PROPERTIES* prop=NULL)
	{ 
        return CImgListInCache::PushBack( CImgInfo(iW, iH, T::Bands(), 
                             sizeof(T::ELT), CCompositeImg<T>::TypeIs()), prop);
    }
	...
	{ 
        return CImgListInCache::PushBack( 
			CLayerImgInfo( CImgInfo(iW, iH, T::Bands(), sizeof(T::ELT), 
                                    CCompositeImg<T>::TypeIs()),
						   iCompositeW, iCompositeH, origin, bCompWrapX, bCompWrapY ), 
            prop );
    }
};
