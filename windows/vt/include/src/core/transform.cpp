...
CRasterBlockMap::GetBlockItems(CRect& rct, int& iPrefetchCount, int iBlockIndex)
{
	...
	iPrefetchCount = (iBlkInMajX==0 && iBlkInMajY==0 &&
					  (iMajRow!=m_iMajCntX-1 || iMajCol!=m_iMajCntY-1))? 1: 0;
...
// 
// Synposis: Implementation of CTaskStatus specific for CTransformTaskTraverse
...
// Class: CTransformTaskTraverse
// 
// Synposis: traverses a transform graph - 
								 ...
								 const VT_TRANSFORM_TASK_OPTIONS* pOpts)
{
	...
	for( UINT i = 0; i < m_vecHeadNodes.size(); i++ )
	{
		...
		for( UINT j = 0; j < pHead->vecTypeSrc.size(); j++ )
		{
...
CTransformTaskTraverse::Merge(const ITaskState* pClone)
{
	...
	for( UINT i = 0; i < m_vecNodes.size(); i++ )
	{
										...
										int iNodeSrcIndex)
{
	...
	for( UINT i = 0; i < uSrcCnt; i++ )
	{
		...
		for( UINT j = 0; j < m_vecNodeSources.size() && !bCommonSrc; j++ )
		{
										  ...
										  const IMAGEREADER_SOURCE* pRdSrc)
{
    ...
    if( vt::CRect(src.infoReader.Rect()).IsRectEmpty() && 
        !pRdSrc->ex.EmptySourceOk() )
    {
        VT_HR_EXIT( E_INVALIDARG );
    }
...
{  
    ...
    for( int i = 0; i < iPrefetchCount; i++ )
	{
		...
		for( UINT j = 0; j < m_vecHeadNodes.size(); j++ )
		{
		...
		for( UINT j = 0; j < m_vecNodeSources.size(); j++ )
		{
