...
BilinearWarpSpan(IN OUT CVec2f* pAddr, const CVec2Img& imgSrcBlk, 
...
CFlowFieldAddressGen::MapDstRectToSrc(const CRect &rctDst)
{
	...
	vt::CRect rctSrc = vt::CRect( F2I(floor(l)), F2I(floor(t)), 
								  F2I(ceil(r))+1,  F2I(ceil(b))+1 );
...
HRESULT
CFlowFieldAddressGen::MapDstSpanToSrc(OUT CVec2f* pSpan, 
...
HRESULT
CFlowFieldAddressGen::MapDstAddrToSrc(IN OUT CVec2f* pSpan, int iSpan)
{
...
HRESULT
CFlowFieldXYAddressGen::MapDstSpanToSrc(OUT CVec2f* pSpan, 
										IN const vt::CPoint &ptDst, int iSpan)
{
	// determine if dst is within the source
...
HRESULT
CFlowFieldXYAddressGen::MapDstAddrToSrc(IN OUT CVec2f*, int)
{
	return E_NOTIMPL;
}
...
HRESULT
C3x3TransformAddressGen::MapDstSpanToSrc(OUT CVec2f* pOut,
...
//#define REFERENCE
	...
	else
	{
        ...
        if ( m_xfrm(2,0) != 0 )
        {
            ...
            if ( fX0 >= 0 && fX0 <= float(iSpan-1) )
            {
                if ( W < 0)
                {
                    ...
                    int beginvalid = F2I(floor(fX0+1.f));
                ...
                else
                {
                    ...
                    int beginnotvalid = F2I(ceil(fX0)); 
...
#endif 
        {
            ...
            // xmm1 = w0 w0 w1 w1
            __m128 xmm1 = _mm_setr_ps(W, W, W+m_xfrm(2,0), W+m_xfrm(2,0));     
            ...
            __m128 xmm3 = _mm_set_ps1(2.f*m_xfrm(2,0));

            for ( ; x < iSpan-1; x+=2, pOut+=2 )
            {
                __m128 xmm4 = xmm0;                 // copy to another reg
...
HRESULT
C3x3TransformAddressGen::MapDstAddrToSrc(OUT CVec2f* pOut, int iSpan)
{
	if( m_bIsAffine )
	{
		for( int x = 0; x < iSpan; x++, pOut++ )
