...
#if defined(VT_WINRT) && !(WINAPI_FAMILY == WINAPI_FAMILY_PHONE_APP) // WBM not available for WP8
HRESULT vt::VtConvertImageToWriteableBitmap(WriteableBitmap^& writeableBitmap, const CImg& imgSrc, bool bBypassCache)
{
	VT_HR_BEGIN();
	...
	{
		writeableBitmap = ref new WriteableBitmap(width, height);
	}
