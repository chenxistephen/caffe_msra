...
floor2SSE(float* pD, const float* pS)
{
	...
	x0f.m128_f32[0]=pS[0];
	x0f.m128_f32[1]=pS[1];
	x0f.m128_f32[2]=x0f.m128_f32[3]=0;
#else
	x0f = _mm_set_ps(pS[0], pS[1], 0, 0);
#endif
	...
	pD[1] = x2f.m128_f32[1];
#else
	pD[0] = SSE2_mm_extract_epf32(x2f, 0);
	pD[1] = SSE2_mm_extract_epf32(x2f, 1);
#endif
}
#endif
...
public:
    ...
    /// </returns>
	virtual HRESULT MapDstSpanToSrc(OUT CVec2f* pSrcCoord, 
	...
	/// - an implementation specific HRESULT on failure
    /// </returns>
	virtual HRESULT MapDstAddrToSrc(IN OUT CVec2f* pSpan, int iSpan) = 0;
...
// 
// Synposis: a collection of functions that map a rectangle in a destination
...
// 
// Synposis: An address generator that creates addresses based on a specified
//           3x3 transform.
// 
//------------------------------------------------------------------------------
