...
public:
	struct TmpType
	{
		...
		void LoadSSE1(float fMin, float fMax)
		{
...
public:
	...
	{
		tmp.LoadSSE1((*param).minVal / float(ElTraits<SrcElT>::MaxVal()), 
			(*param).maxVal / float(ElTraits<SrcElT>::MaxVal()));
	}
...
template
void vt::VtMinMaxSpan(const Byte* pSpan, int iW, float &fMin, float &fMax);
template
void vt::VtMinMaxSpan(const UInt16* pSpan, int iW, float &fMin, float &fMax);
template
void vt::VtMinMaxSpan(const float* pSpan, int iW, float &fMin, float &fMax);
					  ...
					  short iExpMin, short iExpMax)
{
    ...
    if (m_uMantissaBits < m_uTotalBits - m_uExpBits)
    {
        ...
        m_pMantissaLogs = VT_NOTHROWNEW UInt32[(UInt32) (1 << (m_uTotalBits - m_uExpBits))];
    ...
    for (int i = 0; i < 1 << m_uMantissaBits; i++)
		...
		(float) VT_LOG2E * log(1.f + (float) i / (float) (1 << m_uMantissaBits)) );
...
{
    float fSrc[3] = { 0 };
    for (UInt32 i = 0; i < VtMin((UInt32)3, bins); i++)
    {
        fSrc[i] = pfSrc[i];
        pfDst[i] = 0;
    }
